...
GC.normalPositionAttribute = null;
...
demo.prototype.initShaders = function(){
    ...
    GC.normalPositionAttribute = gl.getAttribLocation(this.shaderProgram, "normals");
    gl.enableVertexAttribArray(GC.normalPositionAttribute);
...
demo.prototype.initGeometryBuffers = function(){
  ...
  var normal = [];
    ...
    m.indices.forEach(function(d,i){
        nx = (parseFloat(m.normal[d*3]));
        ny = (parseFloat(m.normal[d*3+1]));
        nz = (parseFloat(m.normal[d*3+2]));
...
//---------
    ...
    //pass the normals to shader
    gl.bindBuffer(gl.ARRAY_BUFFER, GC.normalBuffer);
    gl.vertexAttribPointer(GC.normalPositionAttribute, 3, gl.FLOAT, false, 0, 0);
